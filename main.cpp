...
int main(){
    ...
    sf::RenderWindow window(sf::VideoMode(configuration.WINDOW_X_SIZE, configuration.WINDOW_Y_SIZE), "SFML window");
    ...
    std::unordered_map<std::string, std::string> local_variables;
        ...
        consoleText.setCharacterSize(configuration.FONT_SIZE);
        consoleText.setFillColor(sf::Color::White);
        consoleText.setPosition(static_cast<float>(configuration.RELATIVE_LEFT_X_EDGE), static_cast<float>(configuration.RELATIVE_UPPER_Y_EDGE));

        // Input buffer
        std::string inputBuffer;
        sf::Text inputText;
        inputText.setFont(consoleFont);
        inputText.setCharacterSize(configuration.FONT_SIZE);
        inputText.setFillColor(sf::Color::Green);
        inputText.setPosition(static_cast<float>(configuration.RELATIVE_LEFT_X_EDGE), (static_cast<float>(configuration.RELATIVE_LOWER_Y_EDGE))); // At the bottom of the screen

        // System text
        sf::Text console_system;
        console_system.setFont(consoleFont);
        console_system.setCharacterSize(configuration.FONT_SIZE);
        console_system.setFillColor(sf::Color::Yellow);
        console_system.setPosition(static_cast<float>(configuration.RELATIVE_RIGHT_X_EDGE), static_cast<float>(configuration.RELATIVE_UPPER_Y_EDGE));
    ...
    while (window.isOpen()) {
            ...
            while (window.pollEvent(event)) {
                ...
                if (event.type == sf::Event::TextEntered) {
                    ...
                    } else if (event.text.unicode < configuration.ASCII_COUNT) { // Printable characters
